...
public class CarBuildService {
    ...
    public CarWeb buildCar(String model, String lang) {
        ...
        if (car.isPresent()) {
            ...
            carWeb.setEngineList(engineBuild(lang, car.get(), "engine"));// use detail for limit
    ...
    public List<EngineWrep> engineBuild(String lang, Car car, String check) {
        ...
        engine.forEach(engine1 -> {
            ...
            engineWrep.setImage(engine1.getImage());
    ...
    public ResponseEntity<HomeCarBanner> createHomeCarBanner(Long id, VehicleType vehicleType, String link, boolean visibility, MultipartFile image)
            throws IOException {
        HomeCarBanner domain = HomeCarBanner.builder()
                .link(link)
                .vehicleType(vehicleType)
                .visibility(visibility)
                .build();
        if (null != id && id != 0) {
            Optional<HomeCarBanner> existingBanner =
                    homeCarBannerRepo.findById(id);
            if (existingBanner.isPresent()) {
                domain.setId(id);
                if (image != null && image.getBytes().length > 0) {
                    imageService.deleteFile(existingBanner.get().getImage());
                    FileUpload uploaded = imageService.uploadImage(image, Path.home);
                    domain.setImage(Path.home + uploaded.getFile().getName());
                }
                return ResponseEntity.ok(homeCarBannerRepo.save(domain));
            }
            return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
        }
        if (homeCarBannerRepo.findByVehicleType(vehicleType.name()) != null) {
